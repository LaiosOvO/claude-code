...
QueryEngine 是 claude-code-best 的核心 -- AI Agent 循环。它负责将用户消息发送给 Claude API，解析响应中的工具调用，执行工具，再将结果反馈给 Claude，如此循环直到 Claude 给出最终回答。整个引擎由三层构成：`query.ts`（1732 行 AsyncGenerator 主循环）、`QueryEngine.ts`（1320 行会话编排器）、以及 `query/` 目录下的配置与策略模块。
...
`query.ts` 是 1732 行的 AsyncGenerator 函数，是对话引擎的"心脏"。关键机制包括：
...
`QueryEngine.ts` 是 1320 行的会话编排器，负责管理完整的对话生命周期：
